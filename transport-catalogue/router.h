...
namespace graph {
    ...
    private:
        ...
        void InitializeRoutesInternalData(const Graph& graph) {
            ...
            for (VertexId vertex = 0; vertex < vertex_count; ++vertex) { //по идее не должны выходить за пределы суествующих VertexId, но проитерируется гораздо больше Vertexов
                // из точки в ту же точку дорога занимает 0 расстояния
        ...
        // выкидывает лишнюю вершину на заданном промежутке
        void RelaxRoute(VertexId vertex_from, VertexId vertex_to, const RouteInternalData& route_from,
            ...
            auto& route_relaxing = routes_internal_data_[vertex_from][vertex_to]; // ребро
            const Weight candidate_weight = route_from.weight + route_to.weight; 
            if (!route_relaxing || candidate_weight < route_relaxing->weight) { // создаст более оптимальный путь?
                route_relaxing = { candidate_weight,
                                  route_to.prev_edge ? route_to.prev_edge : route_from.prev_edge }; // если нет т.Ы, куда возвращаться из пункта прибытия, то маршрут линейный?  
                                                                            // и выехав назад из пункта выезда, попадешь в т. Ы?
            }
        }
        ...
        void RelaxRoutesInternalDataThroughVertex(size_t vertex_count, VertexId vertex_through) {
            for (VertexId vertex_from = 0; vertex_from < vertex_count; ++vertex_from) {
                // если есть ребро по таким вершинам
                if (const auto& route_from = routes_internal_data_[vertex_from][vertex_through]) {
                    for (VertexId vertex_to = 0; vertex_to < vertex_count; ++vertex_to) {
                        // то промежуточную вершину можно выкинуть
